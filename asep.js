...
let handler = async (m, { text, reply, isLimit, prefix, naze, setLimit, command }) => {
    ...
    try {
        if (command === 'bratfury') {
            ...
            const url = `https://fastrestapis.fasturl.cloud/maker/furbrat?text=${encodeURIComponent(text)}&style=1&position=center&mode=animated`;
        const response = await axios.get(url, { responseType: "arraybuffer" });
const { Sticker } = require('wa-sticker-formatter');
        // Konversi ke stiker
        const sticker = new Sticker(response.data, {
            pack: `Stiker By ${m.pushName ? m.pushName : author}`,
            author: `${naze.user.name}`,
            type: "image/gif",
        });

        // Kirim stiker ke chat
        const stikerBuffer = await sticker.toBuffer();
        await naze.sendMessage(m.chat, { sticker: stikerBuffer }, { quoted: m });
